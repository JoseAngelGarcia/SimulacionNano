urnas <- function(cumu, part) {  #Encapsular el bloque de codigo
...
replicas <- 5
tiemposSP <- c()
N <- seq(30000,45000,5000) 
K <- seq(300,450,50)
for(part in N) {
  for(cumu in K) { 
    for(rep in 1:replicas) { 
      tsec <- data.matrix(system.time(urnas(cumu,part))[1])[1] 
      tiemposSEC <- c(tiemposSEC, tsec) 
    }
  }
}
...
for(part in N) { 
  for(cumu in K) { 
    tcp <- foreach(r= 1:replicas, .combine = c) %dopar% data.matrix(system.time(urnas(cumu, part))[1])[1] #Paralelismo
    tiemposCP <- c(tiemposP,tp) #se guarda tiempo
    stopImplicitCluster()
  }
}
...
#Prueba estadistica
wilcox.test(tiemposSEC, tiemposP, alternative = "greater")


ndatos <- ((length(K) * length(N)) * replicas)
krep <- c(rep("300",40),rep("350",40),rep("400",40),rep("450",40))
nrep <- c(rep("30000",10), rep("35000",10),rep("40000",10),rep("45000",10))

datosS <- data.frame(Implementacion = rep(c("No paralela", "Paralela"),ndatos), cumu = krep, part = nrep, Tiempos = tiemposSP)
datosC <- data.frame(Implementacion = rep(c("No paralela", "Paralela"),ndatos),cumu = krep, part = nrep, Tiempos = tiemposCP)
library(ggplot2)
